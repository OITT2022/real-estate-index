...
export function ProjectImageManager({ projectId, images: initial }: Props) {
  ...
  async function handleSetPrimary(imageId: string) {
    await setProjectImagePrimary(imageId);
  ...
  async function handleDrop(targetIdx: number) {
    ...
    setImages(reordered);
    setDragIdx(null);
    await reorderProjectImages(projectId, reordered.map((img) => img.id));
  }
        ...
        {images.map((img, idx) => (
          ...
          >
            ...
            <div className="image-actions">
              {!img.isPrimary && <button type="button" onClick={() => handleSetPrimary(img.id)} title="Set as primary">★</button>}
