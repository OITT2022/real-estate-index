...
[previous article was about cameras](https://threejs.org/manual/en/cameras.html) which is
important to have read before you read this article as well as
the [article before that one about lights](https://threejs.org/manual/en/lights.html).
...
is, _for every light that casts shadows all objects marked to cast_
_shadows are rendered from the point of view of the light_. **READ THAT**
...
We'll use some of the code from [the previous article](https://threejs.org/manual/en/cameras.html).
...
a [`MeshBasicMaterial`](https://threejs.org/docs/#api/en/materials/MeshBasicMaterial) as we don't need lighting for the ground.
...
Now we'll make a bunch of spheres. For each sphere we'll create a `base` [`THREE.Object3D`](https://threejs.org/docs/#api/en/core/Object3D) and we'll make both the shadow plane mesh and the sphere mesh
...
We'll go over both of these issues in [another article](https://threejs.org/manual/en/transparency.html).
The shadow is a [`MeshBasicMaterial`](https://threejs.org/docs/#api/en/materials/MeshBasicMaterial) because it doesn't need lighting.
...
We setup 2 lights. One is a [`HemisphereLight`](https://threejs.org/docs/#api/en/lights/HemisphereLight) with the intensity set to 2 to really
brighten things up.
...
The other is a [`DirectionalLight`](https://threejs.org/docs/#api/en/lights/DirectionalLight) so the spheres get some definition
...
move the sphere up and down using [`Math.abs(Math.sin(time))`](https://threejs.org/docs/#api/en/math/Math.abs(Math.sin(time)))
...
Copy and paste the text below into a [stack overflow question](https://threejs.org/manual/examples/resources/editor.html?url=/manual/examples/shadows-fake.html).

```

```

[click here to open in a separate window](https://threejs.org/manual/examples/shadows-fake.html)
...
So, moving on to shadow maps, there are 3 lights which can cast shadows. The [`DirectionalLight`](https://threejs.org/docs/#api/en/lights/DirectionalLight),
the [`PointLight`](https://threejs.org/docs/#api/en/lights/PointLight), and the [`SpotLight`](https://threejs.org/docs/#api/en/lights/SpotLight).

Let's start with the [`DirectionalLight`](https://threejs.org/docs/#api/en/lights/DirectionalLight) with the helper example from [the lights article](https://threejs.org/manual/en/lights.html).
...
Copy and paste the text below into a [stack overflow question](https://threejs.org/manual/examples/resources/editor.html?url=/manual/examples/shadows-directional-light.html).

```

```

[click here to open in a separate window](https://threejs.org/manual/examples/shadows-directional-light.html)
...
of view of the light. In this case there is a camera at the [`DirectionalLight`](https://threejs.org/docs/#api/en/lights/DirectionalLight)
that is looking at its target. Just like [the camera's we previously covered](https://threejs.org/manual/en/cameras.html)
...
a [`CameraHelper`](https://threejs.org/docs/#api/en/helpers/CameraHelper) to the scene.
...
Copy and paste the text below into a [stack overflow question](https://threejs.org/manual/examples/resources/editor.html?url=/manual/examples/shadows-directional-light-with-camera-helper.html).

```

```

[click here to open in a separate window](https://threejs.org/manual/examples/shadows-directional-light-with-camera-helper.html)
...
[`DirectionalLight`](https://threejs.org/docs/#api/en/lights/DirectionalLight) represents light all going in a parallel direction, the
[`DirectionalLight`](https://threejs.org/docs/#api/en/lights/DirectionalLight) uses an [`OrthographicCamera`](https://threejs.org/docs/#api/en/cameras/OrthographicCamera) for its shadow camera.
We went over how an [`OrthographicCamera`](https://threejs.org/docs/#api/en/cameras/OrthographicCamera) works in [the previous article about cameras](https://threejs.org/manual/en/cameras.html).

Recall an [`OrthographicCamera`](https://threejs.org/docs/#api/en/cameras/OrthographicCamera) defines
...
We'll also use the `MinMaxGUIHelper` we created in the [camera article](https://threejs.org/manual/en/cameras.html)
...
Copy and paste the text below into a [stack overflow question](https://threejs.org/manual/examples/resources/editor.html?url=/manual/examples/shadows-directional-light-with-camera-gui.html).

```

```

[click here to open in a separate window](https://threejs.org/manual/examples/shadows-directional-light-with-camera-gui.html)
...
![](https://threejs.org/manual/resources/images/low-res-shadow-map.png)
...
allowed which is available on the renderer as [`renderer.capabilities.maxTextureSize`](https://threejs.org/docs/#api/en/renderers/WebGLRenderer#capabilities).

Switching to the [`SpotLight`](https://threejs.org/docs/#api/en/lights/SpotLight) the light's shadow camera becomes a [`PerspectiveCamera`](https://threejs.org/docs/#api/en/cameras/PerspectiveCamera). Unlike the [`DirectionalLight`](https://threejs.org/docs/#api/en/lights/DirectionalLight)'s shadow camera
where we could manually set most its settings, [`SpotLight`](https://threejs.org/docs/#api/en/lights/SpotLight)'s shadow camera is controlled by the [`SpotLight`](https://threejs.org/docs/#api/en/lights/SpotLight) itself. The `fov` for the shadow
camera is directly connected to the [`SpotLight`](https://threejs.org/docs/#api/en/lights/SpotLight)'s `angle` setting.
...
from our [article about lights](https://threejs.org/manual/en/lights.html).
...
Copy and paste the text below into a [stack overflow question](https://threejs.org/manual/examples/resources/editor.html?url=/manual/examples/shadows-spot-light-with-camera-gui.html).

```

```

[click here to open in a separate window](https://threejs.org/manual/examples/shadows-spot-light-with-camera-gui.html)

And finally there's shadows with a [`PointLight`](https://threejs.org/docs/#api/en/lights/PointLight). Since a [`PointLight`](https://threejs.org/docs/#api/en/lights/PointLight)
shines in all directions the only relevant settings are `near` and `far`.
Otherwise the [`PointLight`](https://threejs.org/docs/#api/en/lights/PointLight) shadow is effectively 6 [`SpotLight`](https://threejs.org/docs/#api/en/lights/SpotLight) shadows
each one pointing to the face of a cube around the light. This means
[`PointLight`](https://threejs.org/docs/#api/en/lights/PointLight) shadows are much slower since the entire scene must be
...
And of course we need to switch the light to a [`PointLight`](https://threejs.org/docs/#api/en/lights/PointLight).
...
Copy and paste the text below into a [stack overflow question](https://threejs.org/manual/examples/resources/editor.html?url=/manual/examples/shadows-point-light.html).

```

```

[click here to open in a separate window](https://threejs.org/manual/examples/shadows-point-light.html)
